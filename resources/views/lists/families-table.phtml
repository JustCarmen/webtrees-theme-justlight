...
<div class="fam-list m-0 w-100">
	<table id="<?= e($table_id) ?>" class="table table-bordered table-sm datatables">
		<thead>
			<tr>
				<th colspan="14" class="jc-toolbar jc-header-toolbar px-2 pt-2 pb-0">
		...
		<tfoot>
			<tr>
			  <th colspan="14" class="jc-toolbar jc-footer-toolbar px-2 pt-2 pb-0">
		...
		<?php foreach ($families as $family): ?>
			...
			<tr class="<?= $family->isPendingDeletion() ? 'old' : ($family->isPendingAddition() ? 'new' : '') ?>">
				...
				<td data-sort="<?= $family->lastChangeTimestamp(true) ?>">
					<?= FunctionsDate::timestampToGedcomDate($family->lastChangeTimestamp(true))->display() ?>
				</td>
