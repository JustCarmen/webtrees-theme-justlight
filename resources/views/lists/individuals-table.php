...
$("#<?= e($table_id) ?>").dataTable( {
	dom: "<?=  Theme::theme()->datatableMarkup() ?>",
	<?= I18N::datatablesI18N() ?>,
	autoWidth: false,
	processing: true,
	retrieve: true,
...
<div class="indi-list m-0 w-100">
  <table id="<?= e($table_id) ?>" class="table table-bordered table-sm data-tables">
		<thead>
			<tr>
				<th colspan="16" class="jc-toolbar jc-header-toolbar px-2 pt-2 pb-0">
		...
		<tfoot>
			<tr>
				<th colspan="16" class="jc-toolbar jc-footer-toolbar px-2 pt-2 pb-0">
		...
		<tbody>
			...
			<tr class="<?= $individual->isPendingDeletion() ? 'old' : ($individual->isPendingAddition() ? 'new' : '') ?>">
				...
				<td data-sort="<?= $individual->lastChangeTimestamp(true) ?>">
					<?= FunctionsDate::timestampToGedcomDate($individual->lastChangeTimestamp(true))->display() ?>
				</td>
